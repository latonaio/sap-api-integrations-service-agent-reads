...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-service-agent-reads/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) ServiceAgentCollection(serviceAgentID string) {
	data, err := c.callServiceAgentSrvAPIRequirementServiceAgentCollection("ServiceAgentCollection", serviceAgentID)
	if err != nil {
		c.log.Error(err)
		return
	}
